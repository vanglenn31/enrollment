                ...
                <div class="max-w-2xl mx-auto w-full space-y-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">
                            Update the details for <strong>{{ $term->label }}</strong>.
                        </p>
                    </div>

                    {{-- Status Warning for Ended Terms --}}
                    @if($term->status === 'ended')
                        <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 rounded-xl px-4 py-3 text-sm">
                            ⚠️ This term has ended. You can still update dates and labels for record accuracy.
                        </div>
                    @endif
                        ...
                        <form method="POST" action="{{ route('admin.terms.update', $term) }}" class="space-y-5">
                            ...
                            <div>
                                ...
                                <select name="semester"
                                    ...
                                    <option value="1st"    {{ old('semester', $term->semester) === '1st'    ? 'selected' : '' }}>1st Semester</option>
                                    <option value="2nd"    {{ old('semester', $term->semester) === '2nd'    ? 'selected' : '' }}>2nd Semester</option>
                                    <option value="summer" {{ old('semester', $term->semester) === 'summer' ? 'selected' : '' }}>Summer Term</option>
                                </select>
                                @error('semester')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <span class="inline-flex px-3 py-1 rounded-full text-xs font-medium {{ $term->statusBadgeClass() }}">
                                    {{ ucfirst($term->status) }}
                                </span>
                                <p class="text-xs text-gray-400 mt-1">
                                    Status is managed through the terms list (Activate / End Term buttons).
                                </p>
                            </div>
